...
if ok_ui and rotation_settings_ui and type(rotation_settings_ui.new) == "function" then
    ui = rotation_settings_ui.new({
        id = "hcs_fire_mage",
        title = hcs_header("MAGE", "HCS Fire Mage"),
        default_x = 700,
        default_y = 200,
        default_w = 520,
        default_h = 650,
    ...
    ui:add_tab({ id = "core", label = "Core" }, function(t)
        ...
        if t.checkbox_grid then
            t:checkbox_grid({
                ...
                columns = 1,
                elements = {
                    { element = menu.fs_only_instant, label = "Flamestrike: Only when Instant" },
                },
            })
        end
    end)
    ui:add_tab({ id = "cooldowns", label = "Cooldowns" }, function(t)
        if t.checkbox_grid then
            t:checkbox_grid({
                ...
                columns = 2,
                elements = {
                    { element = menu.use_cooldowns, label = "Use Combustion + Rune of Power" },
        ...
        if t.checkbox_grid then
            t:checkbox_grid({
                ...
                elements = {
                    ...
                    { element = menu.use_living_bomb, label = "Living Bomb in AoE" },
